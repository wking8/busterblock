...
export default (state = initialState, action) => {
    ...
    switch (type) {
        ...
        case UPDATE_SEARCH_TERM:
            ...
            const results = state.movieData.map(movie => movie.title.toLowerCase().includes(payload.toLowerCase())
                ? movie : null
                    || movie.actors.toLowerCase().includes(payload.toLowerCase()) ? movie : null
                        || movie.director.toLowerCase().includes(payload.toLowerCase()) ? movie : null);
            return {
                ...state,
                searchTerm: payload,
